...
- **Index** : `.cursorignore` minimal. Resync après changement.
- **Guide** : [CURSOR_WORKFLOW.md](./CURSOR_WORKFLOW.md)
- **Règles** : `agent-core.mdc` + `auto-deploy-vercel.mdc` uniquement.
- **Premier message** : zone + chemins + `npm run test:<zone>`.
...
| **G — App terrain**      | `src/app/m/technician/**`, `TechnicianMobileApp.tsx`                                                           | **Cursor** (session) | `cursor/m-technician-app`              |
