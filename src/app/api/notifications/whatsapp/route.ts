...
export async function POST(request: Request) {
  ...
  let body: { to?: string; message?: string; interventionStatus?: string; address?: string; clientName?: string };
  try {
  ...
  if (!message) {
    return NextResponse.json({ ok: false, error: "Missing message or intervention details" }, { status: 400 });
  }
