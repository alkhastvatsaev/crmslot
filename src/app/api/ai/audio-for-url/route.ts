  ...
  try {
    ...
    if (!rel) {
      return NextResponse.json({
        audio: null,
        decision: { status: "none" as const, updatedAt: null },
      });
    }
    ...
    if (!fs.existsSync(audioAbs) || !fs.statSync(audioAbs).isFile()) {
      const found =
        findUploadedAudioRelativePath(uploadsDir, rel) ??
        findUploadedAudioRelativePath(uploadsDir, path.basename(rel));
      if (!found) {
        return NextResponse.json({
          audio: null,
          decision: { status: "none" as const, updatedAt: null },
    ...
    logger.error("[audio-for-url]", { error: e instanceof Error ? e.message : String(e) });
