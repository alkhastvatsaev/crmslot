...
import { e2eSeedAssignedInterventionAdmin } from "@/features/interventions/server/e2eSeedAssignedIntervention";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await e2eSeedAssignedInterventionAdmin(db, {
      interventionId: body.interventionId,
    });
    return NextResponse.json({ ok: true, ...result });
  } catch (e) {
    logger.error("[e2e/seed-assigned-intervention]", {
