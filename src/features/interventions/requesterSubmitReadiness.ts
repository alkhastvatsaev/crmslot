...
export type RequesterChecklistItem = "profile" | "problem" | "address";
...
export type RequesterSubmitReadiness = {
  ...
  address: boolean;
  allReady: boolean;
  /** Clé i18n — bouton profil grisé quand le centre n'est pas prêt. */
...
export function isRequesterAddressComplete(
  ...
  return addressConfirmed && address.length > 0 && address !== REQUESTER_GEOLOC_ADDRESS_PENDING;
}
...
export function computeRequesterSubmitReadiness(params: {
  ...
  const addressOk = isRequesterAddressComplete(params.requestData, params.addressConfirmed ?? true);
  ...
  return {
    ...
    problem: problemOk,
    address: addressOk,
    allReady: profileOk && problemOk && addressOk,
