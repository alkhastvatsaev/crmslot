/**
 * Type pivot du domaine. Référencé par 350+ fichiers via le barrel
 * `@/features/interventions/types`. **Ne pas casser l'API publique** —
 * toute évolution doit rester back-compat (champs optionnels ajoutés,
 * pas de retrait sans plan de migration).
 *
 * Le champ `status` est dupliqué littéralement ici alors que
 * `src/core/api/schemas/interventions.ts` expose un Zod
 * `InterventionStatusSchema` divergent (valeurs différentes). Cette
 * divergence est documentée — ne pas tenter de l'unifier sans audit
 * dédié (hors scope P0-2).
 *
 * Sections internes (banners `// ===`) : repère visuel pour navigation,
 * pas une garantie de regroupement métier strict.
 */
export interface Intervention {
  ...
  // === Client (demande société + particulier) ===
  ...
  /** Lignes facturables saisies par le technicien avant clôture. Shape miroir de `BillingLine` (`billingLineTypes.ts`). */
  billingLines?: {
    description: string;
    quantity: number;
  ...
  // === CRM (client société, sites, devis, SAV) ===
  ...
  // === SLA / skills / géofencing / WhatsApp ===
