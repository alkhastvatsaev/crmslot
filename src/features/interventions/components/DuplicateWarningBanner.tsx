...
import type { DuplicateClientIdentity } from "@/features/interventions/duplicateDetectionCore";
...
export default function DuplicateWarningBanner({
  ...
  useEffect(() => {
    ...
    const q = query(collection(firestore, "interventions"), where("companyId", "==", companyId));

    getDocs(q).then((snap) => {
      const all = snap.docs
        .map((d) => ({ id: d.id, ...d.data() }) as Intervention)
        .filter((iv) => iv.id !== interventionId);
      setMatches(findPotentialDuplicates({ address, problem, client }, all));
    });
  }, [
    interventionId,
    address,
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="text-[12px] font-bold text-amber-800">
          {matches.length} doublon{matches.length > 1 ? "s" : ""} probable
          {matches.length > 1 ? "s" : ""}
        </span>
      </div>
      {matches.map((m) => (
        <div
          key={m.intervention.id}
          className="flex items-center justify-between gap-2 rounded-lg bg-white border border-amber-100 px-3 py-2"
        >
          <div className="flex min-w-0 flex-col">
            ...
            <span className="text-[10px] text-slate-400">
              {m.reason} — {Math.round(m.score * 100)}%
            </span>
          </div>
