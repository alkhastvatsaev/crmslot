...
export default function SmartFormAddressMiniMap({ address, placeLatLng, className }: Props) {
  ...
  const createMap = useCallback(
    (container: HTMLDivElement) => {
      ...
      const map = new mapboxgl.Map({
        ...
        fadeDuration: power.fadeDuration,
        refreshExpiredTiles: power.refreshExpiredTiles,
        maxTileCacheSize: Math.min(power.maxTileCacheSize, 32),
        renderWorldCopies: false,
        respectPrefersReducedMotion: power.respectPrefersReducedMotion,
      });
  ...
  const mapInstance = useMapboxMiniMapLifecycle(containerRef, createMap, [token], {
  ...
  }, [address, placeLatLng, token, mapInstance]);
