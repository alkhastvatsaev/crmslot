...
export const chatbotToolsGemini: FunctionDeclaration[] = [
  ...
  },
  {
    ...
    description: "Récupère la liste des interventions/dépannages qui ne sont pas terminées aujourd'hui ou en attente.",
    parameters: {
      type: Type.OBJECT,
      properties: {
  ...
  },
  {
    ...
    description: "Crée une nouvelle intervention (dépannage ou chantier). DEMANDER TOUJOURS UNE CONFIRMATION AVANT D'APPELER CET OUTIL.",
    parameters: {
      type: Type.OBJECT,
      properties: {
  ...
  },
  {
    ...
    parameters: {
      type: Type.OBJECT,
      properties: {
        itemName: { type: Type.STRING, description: "Nom de la pièce (ex: cylindre, serrure multipoint)" },
      },
      required: ["itemName"],
    },
  }
...
export async function executeChatbotTool(toolName: string, input: Record<string, unknown>, companyId: string) {
  const db = getAdminDb();
  ...
  try {
    switch (toolName) {
      ...
      case "get_active_interventions": {
        ...
        return snap.docs.map(d => {
          const data = d.data();
          return { id: d.id, title: data.title, status: data.status, clientName: data.clientName, urgency: data.urgency };
        });
      }

      case "get_pending_quotes": {
        const snap = await db.collection("companies").doc(companyId).collection("quotes")
          .where("status", "==", "draft")
          .limit(10)
          .get();
      ...
      case "create_intervention": {
        ...
        return { success: true, interventionId: ref.id, message: "Intervention créée avec succès." };
      }
      ...
      case "check_stock": {
        ...
        const results = snap.docs
          .map(d => ({ id: d.id, ...d.data() }))
          .filter((s: Record<string, unknown>) => (typeof s.description === "string" ? s.description : "").toLowerCase().includes(query));
    ...
    console.error("Erreur exécution tool Chatbot", err);
