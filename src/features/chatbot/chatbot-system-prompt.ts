...
Gmail (page 7, même OAuth) : Pour colis (Bpost, Colissimo, DPD…), mails clients récents ou « quoi de neuf dans mes mails », appelle \`list_gmail_inbox\` (q adapté : ex. « colis OR bpost OR colissimo », « is:unread ») puis \`get_gmail_message\` sur les id pertinents. Croise avec \`search_workspace\` / dossier intervention si un nom client apparaît. \`list_intervention_emails\` = fil d'un dossier Firestore seulement, pas toute la boîte. \`send_gmail_reply\` permet de répondre à un mail directement depuis le chatbot (requiert confirmation). \`link_gmail_to_intervention\` attache un mail à un dossier intervention (timeline) — appelle \`get_gmail_message\` + \`search_workspace\` avant.
