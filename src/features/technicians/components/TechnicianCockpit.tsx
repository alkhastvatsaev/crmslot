...
export default function TechnicianCockpit() {
  ...
  const [materialModal, setMaterialModal] = useState<{ open: boolean; templateId?: string; scannedParts?: import('@/features/materials/types').MaterialOrderPart[] }>({ open: false });
  const [billingModal, setBillingModal] = useState(false);
  const [finishModal, setFinishModal] = useState<{ open: boolean; mission: Mission | null }>({ open: false, mission: null });
  ...
  const handleSubmitOrder = async (_parts: MaterialOrderPart[], _urgency: "low" | "normal" | "high") => {
  ...
  const handleStartStop = (mission?: { id: string; client: string; type?: string; address?: string }) => {
    ...
    <div className="w-screen h-screen flex bg-slate-900 overflow-hidden font-sans">
      ...
      {materialModal.open && (
        <div
          ...
          onClick={(e) => { if (e.target === e.currentTarget) setMaterialModal({ open: false }); }}
        >
      ...
      {billingModal && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-6"
          onClick={(e) => { if (e.target === e.currentTarget) setBillingModal(false); }}
        >
          <div className="w-full max-w-lg bg-white rounded-[24px] p-6 shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              {BILLING_TEMPLATES.map(tpl => {
                const totalCents = tpl.lines.reduce((sum, l) => sum + l.unitPriceCents * l.quantity, 0);
                return (
                  <div key={tpl.id} className="border border-slate-200 rounded-2xl p-4 hover:border-blue-200 hover:bg-blue-50/30 transition-colors">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <p className="font-semibold text-slate-800 text-sm">{tpl.name}</p>
                        <p className="text-xs text-slate-400 mt-0.5">{tpl.lines.length} ligne{tpl.lines.length > 1 ? 's' : ''}</p>
                      </div>
