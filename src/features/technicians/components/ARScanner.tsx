  ...
  useEffect(() => {
    ...
    async function startCamera() {
      try {
        ...
        logger.warn("Camera access denied or unavailable", {
    ...
    <div className="fixed inset-0 z-50 bg-black flex flex-col font-['Outfit']">
      ...
      <div className="absolute inset-0 flex flex-col justify-between p-6">
        <div className="flex justify-between items-center z-10">
          ...
          <button
            type="button"
            data-testid="ar-scanner-close"
            onClick={onClose}
            className="p-3 bg-white/80 backdrop-blur-xl rounded-full text-slate-900 border border-white/50 shadow-lg"
          >
            <X className="w-6 h-6" />
          </button>
        </div>
        ...
        <div className="bg-white/90 backdrop-blur-2xl border border-white rounded-[40px] p-8 z-10 transition-all duration-500 shadow-[0_-24px_48px_-12px_rgba(0,0,0,0.2)]">
          ...
          <div className="space-y-4 mt-6">
            <div className="flex justify-between items-center text-sm font-medium">
              ...
              <span
                className={`font-bold ${phase === "success" ? "text-slate-900" : "text-slate-300"}`}
              >
            ...
            <div className="flex justify-between items-center text-sm font-medium">
              ...
              <span
                className={`font-bold ${phase === "success" ? "text-blue-600" : "text-slate-300"}`}
              >
