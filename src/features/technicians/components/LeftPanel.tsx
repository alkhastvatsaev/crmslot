...
const STATUS_CONFIG: Record<TechnicianStatus, { label: string; ring: string; dot: string; icon: React.ComponentType<{ className?: string }> }> = {
  available:    { label: 'Disponible', ring: 'ring-emerald-500', dot: 'bg-emerald-500',  icon: CheckCircle2 },
...
export default function LeftPanel({
  ...
  const QUICK_ACTIONS = [
    { icon: Package,   label: 'Commander matériel', color: 'text-blue-400',   onClick: onOpenMaterialOrder },
    ...
    { icon: PhoneCall, label: 'Appel urgence',        color: 'text-red-400',    onClick: () => window.open('tel:+3221234567') },
  ];
  ...
  return (
    <div className="w-[68px] h-full bg-zinc-950 border-r border-white/10 flex flex-col items-center py-5 gap-5">
      ...
      <div className="relative">
        <div className={`w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-violet-600 flex items-center justify-center font-black text-white text-lg select-none ring-2 ${STATUS_CONFIG[status].ring}`}>
          T
        </div>
        <span className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-zinc-950 ${STATUS_CONFIG[status].dot} animate-pulse`} />
      </div>
      ...
      <div className="flex flex-col gap-1.5 w-10">
        {STATUSES.map(s => {
          ...
          return (
            ...
            >
              <Icon className={`w-4.5 h-4.5 ${status === s ? STATUS_CONFIG[s].dot.replace('bg-', 'text-') : ''}`} />
            </button>
          );
        })}
      </div>
      ...
      <div className="flex flex-col items-center gap-3 pb-1">
        ...
        <div className="text-center" title={`${missionsDone} terminées`}>
          <p className="text-xl font-black text-emerald-400 tabular-nums leading-none">{missionsDone}</p>
        ...
        <div className="text-center" title={`${missionsToday - missionsDone} restantes`}>
          <p className="text-xl font-black text-amber-400 tabular-nums leading-none">{missionsToday - missionsDone}</p>
