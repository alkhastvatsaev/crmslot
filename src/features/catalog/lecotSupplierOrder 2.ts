...
import { placeOrderViaPlaywright } from "@/features/catalog/lecotPlaywrightScraper";
...
export async function submitLecotSupplierOrder(params: {
  ...
  // ── 2. Playwright — checkout invité sur lecot.be ────────────────────────────
  if (params.companyId) {
    try {
      const guestInfo = await loadLecotGuestInfo(params.companyId);
      if (guestInfo.email && guestInfo.phone) {
        const result = await placeOrderViaPlaywright(lines, guestInfo);
        if (result.ok) return { ok: true, source: "playwright", orderId: result.orderId };
      }
    } catch {
